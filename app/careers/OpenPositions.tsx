...
const OpenPositions = () => {
  ...
  return (
    <section id="open-positions" className="py-28 md:py-36 bg-white relative">
      {/* Faint grid texture */}
      <div
        ...
        >
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-11 pl-11 pr-4 text-sm bg-[#fafaf8] border border-[#e2e8f0] rounded-lg text-[#111827] placeholder:text-[#718096] focus:outline-none focus:border-[#0098AF] transition-colors"
            />
          </div>
          ...
          <div className="relative w-full sm:w-52">
            <button
              ...
              className="w-full h-11 px-4 text-sm bg-[#fafaf8] border border-[#e2e8f0] rounded-lg text-[#111827] flex items-center justify-between gap-2 focus:outline-none focus:border-[#0098AF] transition-colors"
            >
              <span className="truncate">{selectedLabel}</span>
              <ChevronDown
                className={`w-4 h-4 text-[#718096] flex-shrink-0 transition-transform duration-200 ${isDropdownOpen ? "rotate-180" : ""}`}
              />
            </button>
            <AnimatePresence>
              {isDropdownOpen && (
                <motion.div
                  ...
                  className="absolute top-12 left-0 right-0 z-50 bg-white border border-[#e2e8f0] rounded-lg shadow-lg overflow-hidden"
                >
                  {["all", ...departments].map((dept) => (
                    <button
                      ...
                      className={`w-full px-4 py-2.5 text-left text-sm transition-colors hover:bg-[#f0f9fb] ${
                        selectedDepartment === dept
                          ? "text-[#0098AF] font-medium bg-[#f0f9fb]"
                          : "text-[#111827]"
                      }`}
                    >
            ...
            {filteredJobs.map((job, index) => (
              ...
              >
                <Link href={`/careers/${job.id}`}>
                  <div className="group grid grid-cols-1 md:grid-cols-[1fr_160px_140px_44px] gap-2 md:gap-4 items-center px-5 py-5 border-b border-[#e2e8f0] hover:bg-[#fafaf8] transition-colors duration-200 cursor-pointer">
                    ...
                    <div className="hidden md:flex items-center justify-end">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-[#718096] group-hover:text-[#0098AF] group-hover:bg-[#0098AF]/10 transition-all duration-200">
